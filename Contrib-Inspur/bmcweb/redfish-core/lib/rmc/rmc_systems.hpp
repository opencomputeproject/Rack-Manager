...
class SystemsCollection : public Node
{
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        systemArrary.push_back({{"@odata.id", "/redfish/v1/Systems/system"}});
...
class Systems : public Node
{
...
#else
        Systems(CrowApp &app) : Node(app, "/redfish/v1/Systems/<str>/", std::string())
#endif
    {
        entityPrivileges = {
            {boost::beast::http::verb::get, {{"Login"}}},
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        res.jsonValue["ProcessorSummary"]["Status"]["State"] = "Disabled";
        res.jsonValue["MemorySummary"]["TotalSystemMemoryGiB"] = uint64_t(96);
        res.jsonValue["MemorySummary"]["Status"]["State"] = "Disabled";
        ...
        res.jsonValue["Storage"] = {
            {"@odata.id", "/redfish/v1/Systems/" + name + "/Storage" }};
